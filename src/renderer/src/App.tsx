...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Mirror user messages sent from a paired phone (RemoteServer) into the
    // desktop chat so the user sees what they typed remotely.
    const unsubUserMsg = window.api.agent.onUserMessage(({ sessionId, content }) => {
      addUserMessage(sessionId, content)
      setProcessing(sessionId, true)
    })
