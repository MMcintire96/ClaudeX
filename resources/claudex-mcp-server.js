 ...
 * the Claude CLI's MCP protocol to the ClaudeX bridge HTTP server.
 *
 * Environment variables:
 *   CLAUDEX_BRIDGE_PORT  — bridge server port on localhost
 *   CLAUDEX_BRIDGE_TOKEN — bearer token for auth
 *   CLAUDEX_PROJECT_PATH — current project path
 *   CLAUDEX_SESSION_ID   — this session's unique ID (for inter-session messaging)
 *
...
const BRIDGE_PORT = process.env.CLAUDEX_BRIDGE_PORT
const BRIDGE_TOKEN = process.env.CLAUDEX_BRIDGE_TOKEN
const PROJECT_PATH = process.env.CLAUDEX_PROJECT_PATH || ''
const SESSION_ID = process.env.CLAUDEX_SESSION_ID || ''

if (!BRIDGE_PORT || !BRIDGE_TOKEN) {
  process.stderr.write('claudex-mcp-server: CLAUDEX_BRIDGE_PORT and CLAUDEX_BRIDGE_TOKEN are required\n')
  process.exit(1)
}
  ...
  switch (method) {
    case 'initialize':
      return makeResponse(msg.id, {
        ...
        capabilities: { tools: {} },
        serverInfo: { name: 'claudex-bridge', version: '1.0.0' }
      })
...
process.stdin.on('data', (chunk) => {
  ...
  while ((newlineIdx = inputBuffer.indexOf('\n')) !== -1) {
    ...
    try {
      ...
      process.stderr.write('claudex-mcp-server: invalid JSON: ' + line.slice(0, 200) + '\n')
      continue
    }
    ...
    handleMessage(msg).then((response) => {
      ...
      process.stderr.write('claudex-mcp-server: handler error: ' + err.message + '\n')
      if (msg.id !== undefined && msg.id !== null) {
